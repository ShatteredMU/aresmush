...
module AresMUSH
  module Magic
    ...
    def self.cast_attack_target(caster_name, combatant, target, called_shot = nil, result)
        ...
        combatant.log "#{caster_name} 's margin pre 'determine attack margin' margin=#{margin}"
        combatant.log "#{caster_name} 's net successes pre 'determine attack margin' net successes=#{attacker_net_successes}"
        margin = FS3Combat.determine_attack_margin(combatant, target, mod = 0, called_shot = nil, mount_hit = false, result)
